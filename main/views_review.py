...
def review(request):
    ...
    if request.method == "POST":
        required_fields = ["academic_year", "semester", "content", "is_anonym", "tags"]
...
def ds_review(request):
    ...
    """
    if request.method == "GET":
        reviews = Review.objects.filter(hate_speech_status="WAITING").filter(
            is_active=True
        )
        if reviews.exists():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            logger.error(
                "Failed to update review, request data {}".format(request.data)
            )
            return response(
                error="Failed to update review, error message: {}".format(e),
                status=status.HTTP_404_NOT_FOUND,
            )
...
def get_reviews_by_author(request, user_id, page, code):
    if code != None:
        ...
        reviews = (
            Review.objects.filter(user=user_id)
