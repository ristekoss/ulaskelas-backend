...
from .serializers import (
    ...
    UserCumulativeGPASerializer,
    UserGPASerializer,
    CourseForSemesterSerializer,
    SemesterWithCourseSerializer,
)

from .utils import (
    get_recommended_score,
    get_score,
    response,
    update_course_score,
    validate_body,
    check_notexist_and_create_user_cumulative_gpa,
    validate_body_minimum,
    add_semester_gpa,
    delete_semester_gpa,
    update_semester_gpa,
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if not isinstance(given_semesters, list) or not given_semesters:
                return response(
                    error="given_semesters should be a non-empty list",
...
def gpa_calculator_with_semester(request, given_semester):
    user = Profile.objects.get(username=str(request.user))
    user_cumulative_gpa = check_notexist_and_create_user_cumulative_gpa(user)
    user_gpa = UserGPA.objects.filter(
        userCumulativeGPA=user_cumulative_gpa, given_semester=given_semester
    ).first()
    ...
    if request.method == "GET":
        ...
        return response(
            data=UserGPASerializer(user_gpa).data, status=status.HTTP_200_OK
        )
...
def course_semester(request):
    ...
    if request.method == "GET":
        is_valid = validate_params(request, ["given_semester"])

        if is_valid != None:
            return is_valid
        given_semester = request.query_params.get("given_semester")
        ...
        if user_gpa is None:
            return response(
                error="There is no user_gpa with given_semester={}".format(
                    given_semester
                ),
                status=status.HTTP_404_NOT_FOUND,
            )
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if not isinstance(course_ids, list) or not course_ids:
                return response(
                    error="course_ids should be a non-empty list",
                    status=status.HTTP_400_BAD_REQUEST,
                )
        except Exception as e:
            return response(
        ...
        if semester is None:
            return response(
                error="No such semester with given_semester={}".format(given_semester),
                status=status.HTTP_404_NOT_FOUND,
            )
        ...
        for course_id in course_ids:
            ...
            course_semester = CourseSemester.objects.filter(
        ...
        return response(
            data={
                "updated_course": SemesterWithCourseSerializer(semester).data,
...
def course_semester_with_course_id(request, course_id):
    ...
    if request.method == "DELETE":
        ...
        course_semester = CourseSemester.objects.filter(
            ...
            calculator__user=user,
            semester__given_semester=given_semester,
        ).first()
        if course_semester is None:
            return response(
                error="No matching course semester", status=status.HTTP_404_NOT_FOUND
            )

        # Update gpa (ip) and cumulative gpa (ipk)
        delete_course_to_semester(
            semester=semester,
            sks=course_semester.course.sks,
            score=get_score(course_semester.calculator.total_score),
        )
        delete_semester_gpa(
            user_cumulative_gpa=user_cumulative_gpa,
...
def course_component(request):
    ...
    if request.method == "GET":
        ...
        if calculator is None:
            return response(
                error="Calculator not found", status=status.HTTP_404_NOT_FOUND
            )
    ...
    if request.method == "POST":
        ...
        if calculator is None:
            return response(
                error="There is no calculator with id={}".format(calculator_id),
                status=status.HTTP_404_NOT_FOUND,
            )
        ...
        score_component = ScoreComponent.objects.create(
        ...
        update_course_score(
            ...
            cur_sks=course.sks,
            cur_score=get_score(calculator.total_score),
        )
    ...
    if request.method == "DELETE":
        ...
        if score_component is None:
            return response(
                error="Score component not found", status=status.HTTP_404_NOT_FOUND
            )

        calculator = Calculator.objects.filter(id=score_component.calculator.id).first()
        if calculator is None:
            return response(
                error="There is no calculator with id={}".format(
                    score_component.calculator.id
                ),
                status=status.HTTP_404_NOT_FOUND,
            )
        course_semester = CourseSemester.objects.filter(calculator=calculator).first()
...
def course_subcomponent(request):
    ...
    if request.method == "GET":
        ...
        score_subcomponent_details = ScoreSubcomponent.objects.filter(
            score_component=score_component
        ).order_by("subcomponent_number")
        list_score_subcomponent = [
            score_subcomponent.subcomponent_score
            for score_subcomponent in score_subcomponent_details
        ]
        return response(
            data={
    ...
    if request.method == "POST":
        is_valid = validate_body(
            request, ["calculator_id", "name", "weight", "frequency", "scores"]
        )
        ...
        try:
            ...
            if not isinstance(scores, list) or not scores:
                return response(
                    error="scores should be a non-empty list",
                    status=status.HTTP_400_BAD_REQUEST,
                )
        except Exception as e:
            return response(
        ...
        score_component = ScoreComponent.objects.create(
            calculator=calculator, name=name, weight=weight, score=0
        )

        component_total_score = 0
        for index, score in enumerate(scores):
            score_subcomponent = ScoreSubcomponent.objects.create(
                score_component=score_component,
    ...
    if request.method == "PUT":
        is_valid = validate_body(
            request, ["score_component_id", "name", "weight", "frequency", "scores"]
        )
        ...
        if score_component is None:
            return response(
                error="There is no score_component with id={}".format(
                    score_component_id
                ),
                status=status.HTTP_404_NOT_FOUND,
            )
        ...
        if course_semester is None:
            return response(
                error="No Course Semester", status=status.HTTP_404_NOT_FOUND
            )
        semester = course_semester.semester
        course = course_semester.course
        ...
        for index in range(1, frequency + 1):
            if index <= previous_frequency:  # Update previous subcomponent
                current_score_subcomponent = ScoreSubcomponent.objects.filter(
                    score_component=score_component, subcomponent_number=index
                ).first()
