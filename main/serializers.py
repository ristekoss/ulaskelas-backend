...
class CourseSerializer(serializers.ModelSerializer):
    ...
    rating_understandable = serializers.SerializerMethodField(
        "get_rating_understandable"
    )
    rating_fit_to_credit = serializers.SerializerMethodField("get_rating_fit_to_credit")
    rating_fit_to_study_book = serializers.SerializerMethodField(
        "get_rating_fit_to_study_book"
    )
    ...
    def get_review_count(self, obj):
        return (
            obj.reviews.filter(is_active=True)
            .filter(hate_speech_status="APPROVED")
            .count()
        )
    ...
    def get_top_tags(self, obj):
        ...
        top_tags = [
            k
            for k, v in sorted(tag_count.items(), key=lambda tag: tag[1], reverse=True)
        ]
        return top_tags[:3]
    ...
    def get_all_rating(self, obj):
        obj.ratings = (
            obj.reviews.filter(is_active=True)
            .filter(hate_speech_status="APPROVED")
            .filter(rating_understandable__gte=1)
            .aggregate(
    ...
    def get_rating_average(self, obj):
        ...
        rating_total = (
            (obj.ratings.get("rating_understandable") or 0.0)
...
class ReviewSerializer(serializers.ModelSerializer):
    ...
    def get_course_review_count(self, obj):
        return (
            obj.course.reviews.filter(is_active=True)
            .filter(hate_speech_status="APPROVED")
            .count()
        )
    ...
    def get_rating_average(self, obj):
        return (
            (obj.rating_understandable or 0)
            + (obj.rating_fit_to_credit or 0)
            + (obj.rating_fit_to_study_book or 0)
            + (obj.rating_beneficial or 0)
            + (obj.rating_recommended or 0)
        ) / 5
...
class TanyaTemanProfileSerializer(serializers.ModelSerializer):
    ...
    def get_cleaned_program(self, obj):
        ...
        if "(" in cleaned_study_program:
            cleaned_study_program = cleaned_study_program[
                : cleaned_study_program.find("(")
            ]
