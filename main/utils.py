...
def validate_params(request, params):
    for param in params:
        ...
        if res is None:
            return response(
                error="{} is required".format(param), status=status.HTTP_404_NOT_FOUND
            )
    return None
...
def validate_body(request, attrs):
    for attr in attrs:
        ...
        if res is None:
            return response(
                error="{} is required".format(attr), status=status.HTTP_404_NOT_FOUND
            )
    return None
...
def get_paged_obj(objs, page, _sort_by_id=True):
    if _sort_by_id:
...
def validate_body_minimum(request, attrs):
    ...
    for attr in attrs:
        ...
        if res is not None:
            return None
    return response(
        error="None of the following attributes are present in the request data: {}.".format(
            attrs
        ),
...
def update_course_score(
    user_cumulative_gpa: UserCumulativeGPA,
    semester: UserGPA,
    prev_sks: int,
    prev_score: float,
    cur_sks: int,
...
def add_semester_gpa(
    user_cumulative_gpa: UserCumulativeGPA, total_sks: int, semester_gpa: float
):
...
def update_cumulative_gpa(user_cumulative_gpa: UserCumulativeGPA):
    if user_cumulative_gpa.total_sks == 0:
        ...
        user_cumulative_gpa.cumulative_gpa = (
            user_cumulative_gpa.total_gpa / user_cumulative_gpa.total_sks
        )
...
def get_null_sum_from_component(score_component: ScoreComponent) -> float:
    frequency = ScoreSubcomponent.objects.filter(
        score_component=score_component
    ).count()
    null_sum = 0.0
    score_subcomponents = ScoreSubcomponent.objects.filter(
        score_component=score_component
    )
    for score_subcomponent in score_subcomponents:
        null_contribution = (
            0
            if score_subcomponent.subcomponent_score != None
            else score_component.weight / frequency
        )
