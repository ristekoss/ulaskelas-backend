...
class Review(models.Model):
    ...
    hate_speech_status = models.CharField(
        choices=HateSpeechStatus.choices, max_length=20
    )
...
class UserGPA(models.Model):
    ...
    class Meta:
        constraints = [
            UniqueConstraint(
                fields=["userCumulativeGPA", "given_semester"],
                name="unique_userCumulativeGPA_given_semester",
            )
        ]
...
class ScoreSubcomponent(models.Model):
    ...
    class Meta:
        constraints = [
            UniqueConstraint(
                fields=["score_component", "subcomponent_number"], name="unique_number"
            )
        ]
...
class Question(models.Model):
    ...
    verification_status = models.TextField(
        choices=VerificationStatus.choices, default=VerificationStatus.WAITING
    )
    ...
    def image_tag(self, obj):
        return format_html(
            '<img src="{}" style="max-width:200px; max-height:200px"/>'.format(
                get_attachment_presigned_url(obj.attachment)
            )
        )
