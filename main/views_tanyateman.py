...
def tanya_teman(request):
    ...
    if request.method == "POST":
        ...
        if course_id != None:
            ...
            if course is None:
                return response(
                    error="No such Course", status=status.HTTP_404_NOT_FOUND
                )
        else:
            course = None
        ...
        if is_anonym > 1 or is_anonym < 0:
            return response(
                error="is_anonym should be between 0 and 1",
    ...
    if request.method == "DELETE":
        ...
        if not id.isnumeric():
            return response(
                error="id should be a number", status=status.HTTP_400_BAD_REQUEST
            )
        return tanya_teman_with_id(request, id)
    ...
    if request.method == "PUT":
        ...
        if is_like is None:
            return response(
                error="The only allowed PUT request for /tanya-teman is to like/unlike a Question.",
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if question is None:
            return response(
                error="No matching question", status=status.HTTP_404_NOT_FOUND
            )

        content_type = ContentType.objects.get_for_model(Question)
        question_like = LikePost.objects.filter(
            content_type=content_type, object_id=id, user=user
        ).first()
        if question_like is None:
            LikePost.objects.create(user=user, content_type=content_type, object_id=id)
...
def jawab_teman(request):
    ...
    if request.method == "PUT":
        ...
        if is_like is None:
            return response(
                error="The only allowed PUT request for /jawab-teman is to like/unlike an Answer.",
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if answer is None:
            return response(
                error="No matching answer", status=status.HTTP_404_NOT_FOUND
            )

        content_type = ContentType.objects.get_for_model(Answer)
        answer_like = LikePost.objects.filter(
            content_type=content_type, object_id=id, user=user
        ).first()
...
def tanya_teman_with_id(request, id):
    ...
    if request.method == "DELETE":
        if question.user.pk != user.pk:
            return response(
                error="You are not allowed to delete other person's question",
...
def tanya_teman_paged(request, questions, is_history):
    ...
    return response_paged(data={"questions": list_questions}, total_page=total_page)
...
def upload_attachment(attachment_file):
    ...
    if not extension:
        return "error", response(
            error="File extension not found.", status=status.HTTP_400_BAD_REQUEST
        )

    if str(extension) not in ["jpg", "jpeg", "png"]:
        return "error", response(
            error="File must be jpg, jpeg, or png.", status=status.HTTP_400_BAD_REQUEST
        )

    if not folder_prefix:
        return "error", response(
            error="Folder prefix not found.", status=status.HTTP_400_BAD_REQUEST
        )
...
def add_is_liked(user, posts, is_question):
    ...
    if is_question:
        ...
        content_type = ContentType.objects.get_for_model(Answer)
    for post in posts:
        post_like = LikePost.objects.filter(
